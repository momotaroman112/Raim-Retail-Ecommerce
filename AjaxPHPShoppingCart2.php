<?php
	...
	if($strProductID != "" and $strQty  != "")
	{
		...
		if($numrow == ''){
			...
			$strSQL2 = "update product_detail_join set SID = '".session_id()."' WHERE product_id = '".$strProductID."' or type_join = '".$strProductID."' ";
		...
		if($row_ty[protype_id] == 2 || $row_ty[protype_id] == 1 || $row_ty[protype_id] == 3 || $row_ty[protype_id] == 7 || $row_ty[protype_id] == 8 || $row_ty[protype_id] == 9 || $row_ty[protype_id] == 6){
			...
			$strProductID = $objResult_4[id_product];
...
if($_GET["Action"] == "Del")
{
	...
	$strSQL2 = "update product_detail_join set SID = '' WHERE product_id = '".$_GET["id"]."' or type_join = '".$_GET["id"]."' ";
	$objQuery2 = mysql_query($strSQL2);
	
	$strSQL_num = "SELECT * FROM tb_orders  WHERE SID = '".session_id()."' ";
	$objQuery_num = mysql_query($strSQL_num);
	$numro = mysql_num_rows($objQuery_num);
	if($numro == ''){
		session_start(); // ลบค่า session
		session_destroy();
	}
	//echo session_id();
?>
