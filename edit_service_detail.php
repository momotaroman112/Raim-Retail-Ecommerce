           ...
           <div class="cart-view-table">
			<h2>แก้ไขรายละเอียดแจ้งซ่อมสินค้า</h2>
            	<?
					...
					$row_ser = mysql_fetch_array($stmt_ser);
					
					$sql = "SELECT * FROM service_detail WHERE serdetail_id = '".$_GET['serdetail_id']."' ";
			  ...
			  <form action="update_service_detail.php" class="form-horizontal" role="form" method="post"  enctype="multipart/form-data">		
              		<input type="hidden" value="<?=$row[service_id];?>" name="service_id" required />
                    <input type="hidden" value="<?=$row[serdetail_id];?>" name="serdetail_id" required />
